...
@section('title', 'Edit Atraksi & Destinasi')
@section('subtitle', 'Perbarui informasi detail lokasi, sejarah, dan jam operasional.')

@section('content')

    <div class="mb-6">
        <a href="{{ route('admin.attractions.index') }}" class="inline-flex items-center gap-2 text-sm font-bold text-brand-muted dark:text-white/50 hover:text-brand-accent dark:hover:text-brand-gold transition-colors no-underline">
        ...
        <div class="bg-white dark:bg-[#1E1212] p-6 sm:p-10 rounded-[2.5rem] border border-gray-100 dark:border-white/5 shadow-sm">
            ...
            <div class="mb-10">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    ...
                    <div>
                        <label class="block text-sm font-bold text-brand-text dark:text-white mb-2 text-emerald-600 dark:text-emerald-400">Jam Buka</label>
                        <div class="relative">
                    ...
                    <div>
                        <label class="block text-sm font-bold text-brand-text dark:text-white mb-2 text-rose-500">Jam Tutup</label>
                        <div class="relative">
                            <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
            ...
            <div class="mb-8">
                ...
                <div>
                    <label class="block text-sm font-bold text-brand-text dark:text-white mb-3">Tambah Foto Galeri Baru</label>
                    <label class="flex flex-col items-center justify-center w-full h-32 rounded-2xl border-2 border-dashed border-gray-300 dark:border-white/20 bg-gray-50 hover:bg-gray-100 dark:bg-[#0A0505] dark:hover:bg-white/5 transition-all cursor-pointer">
                        ...
                        <input type="file" id="gallery_upload" name="gallery[]" class="hidden" accept="image/*" multiple />
                    </label>

                    <div id="gallery-preview-container" class="mt-6 grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-4 hidden">
                    </div>
                </div>
            </div>
            <div class="pt-8 border-t border-gray-100 dark:border-white/10 flex justify-center">
                <button type="submit" class="w-full md:w-auto md:px-16 py-4 rounded-2xl bg-brand-accent dark:bg-brand-gold text-white dark:text-brand-dark font-bold text-sm shadow-xl shadow-brand-accent/30 hover:-translate-y-1 transition-all flex items-center justify-center gap-2">
                    <i class="iconoir-check-circle"></i> Perbarui Atraksi Wisata
                </button>
            </div>
            ...
            if (mainInput) {
                ...
                mainInput.addEventListener('change', function(e) {
                    ...
                    if (file && file.type.startsWith('image/')) {
                        ...
                        reader.onload = (event) => { mainPreview.src = event.target.result; };
            ...
            function updateGalleryUI() {
                ...
                selectedFiles.forEach((file, index) => {
                    ...
                    reader.onload = (event) => {
                        ...
                        div.className = 'relative aspect-square rounded-xl overflow-hidden shadow-sm group border border-gray-200 dark:border-white/10';
                        div.innerHTML = `
