...
@section('content')
        ...
        <div class="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
            ...
            <div class="relative">
                <select id="filterCategory"
                    class="appearance-none w-full sm:w-48 px-4 py-3 rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-[#1E1212] text-brand-text dark:text-white focus:ring-2 focus:ring-brand-accent/20 focus:border-brand-accent dark:focus:ring-brand-gold/30 dark:focus:border-brand-gold outline-none cursor-pointer shadow-sm pr-10 transition-all duration-300">
    ...
    <div
            ...
            <table class="w-full text-left border-collapse whitespace-nowrap">
                <thead>
                    <tr class="bg-gray-50 dark:bg-black/20 border-b border-gray-100 dark:border-white/5">
                        ...
                        <th
                            ...
                            Judul Paket</th>
                        <th
                            class="px-6 py-4 text-[10px] font-extrabold uppercase tracking-widest text-brand-muted dark:text-white/50 text-right">
                            Harga (Rp)</th>
                        <th
                ...
                <tbody class="divide-y divide-gray-50 dark:divide-white/5">
                    ...
                    @foreach ($packages as $package)
                        <tr class="package-row group group hover:bg-gray-50/80 dark:hover:bg-white/5 transition-colors"
                            data-title="{{ strtolower($package->title_line1 . ' ' . $package->title_line2) }}"
                            data-category="{{ strtolower($package->category) }}">
                            ...
                            <td class="px-6 py-4">
                                <p class="text-sm font-bold text-brand-text dark:text-white truncate max-w-[250px]">
                                    {{ $package->title_line1 }} {{ $package->title_line2 }}
                                </p>
                            </td>
                            ...
                            <td class="px-6 py-4 text-center">
                                <span
                                    class="inline-flex items-center px-2.5 py-1 rounded-md bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 text-[10px] font-bold uppercase tracking-wider border border-blue-100 dark:border-blue-500/20">
                                    {{ $package->category }}
                                </span>
                            </td>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <a href="{{ route('admin.packages.edit', $package->id) }}"
                                        class="w-8 h-8 rounded-lg flex items-center justify-center bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 hover:bg-blue-600 hover:text-white dark:hover:bg-blue-500 transition-colors tooltip-trigger">
                                    ...
                                    <form action="{{ route('admin.packages.destroy', $package->id) }}" method="POST">
                                        ...
                                        <button type="submit"
                                            class="w-8 h-8 rounded-lg flex items-center justify-center bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400 hover:bg-amber-500 hover:text-white dark:hover:bg-amber-500 transition-colors tooltip-trigger outline-none border-none cursor-pointer">
        ...
        <div class="p-6 border-t border-gray-100 dark:border-white/5 flex items-center justify-between flex-wrap gap-4">
            <p class="text-xs text-brand-muted dark:text-white/50 font-medium">Menampilkan <span
                    ...
                    class="font-bold text-brand-text dark:text-white">2</span> paket</p>
    ...
    <div id="detailModal"
        ...
        <div id="detailModalCard"
            ...
            <div class="relative w-full h-40 sm:h-48 shrink-0">
                <img src="https://images.unsplash.com/photo-1555396273-367ea4eb4db5?q=80&w=1000" alt="Cover"
                    ...
                    <div class="flex items-center gap-2 mb-2">
                        <span
                            class="px-2 py-0.5 rounded-md bg-brand-gold text-brand-dark text-[10px] font-bold uppercase tracking-wider flex items-center gap-1"><i
                                class="iconoir-star-solid text-[10px]"></i> Best Seller</span>
                        <span
                            class="px-2 py-0.5 rounded-md bg-white/20 backdrop-blur-md text-white text-[10px] font-bold uppercase tracking-wider border border-white/20">2
                            Hari 1 Malam</span>
                    </div>
                    <h3 class="text-2xl sm:text-3xl font-bold text-white leading-tight">Eksplorasi Mendalam <span
                            class="text-brand-gold italic">Kampoeng Naga</span></h3>
            ...
            <div class="p-0 overflow-y-auto custom-scrollbar flex-1 bg-gray-50/30 dark:bg-transparent">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-0 h-full">
                    ...
                    <div class="md:col-span-2 p-6 sm:p-8 border-r border-gray-100 dark:border-white/5 space-y-8">
                        <div>
                            <h4 class="text-xs font-extrabold uppercase tracking-widest text-brand-muted mb-2">Deskripsi
                            </h4>
                            <p class="text-sm text-gray-600 dark:text-white/60 leading-relaxed text-justify">Paket
                                perjalanan unggulan ini tidak dirancang sekadar untuk berlibur, melainkan untuk membawa Anda
                                menyelami romantika kehidupan tempo dulu, di mana harmoni budaya, kuliner legendaris, dan
                                arsitektur otentik melebur menjadi satu.</p>
                        </div>
                        ...
                        <div>
                            <h4
                                ...
                                Ringkasan Itinerary</h4>
                            <div class="space-y-4">
                                <div class="flex gap-4">
                                    <div
                                        class="w-8 h-8 rounded-full bg-brand-accent/10 dark:bg-brand-gold/10 text-brand-accent dark:text-brand-gold flex items-center justify-center font-bold text-xs shrink-0">
                                        H1</div>
                                    <div>
                                        <p class="text-sm font-bold text-brand-text dark:text-white mb-1">Ketibaan &
                                            Petualangan Malam</p>
                                        <ul class="space-y-1">
                                            <li class="text-xs text-gray-500 dark:text-white/50 flex items-center gap-2"><i
                                                    class="iconoir-clock text-brand-accent dark:text-brand-gold"></i> 14:00
                                                - Check-In Hotel</li>
                                            <li class="text-xs text-gray-500 dark:text-white/50 flex items-center gap-2"><i
                                                    class="iconoir-clock text-brand-accent dark:text-brand-gold"></i> 19:00
                                                - Simfoni Kuliner Malam</li>
                                        </ul>
                                    </div>
                                </div>
                                ...
                                <div class="flex gap-4">
                                    <div
                                        ...
                                        H2</div>
                                    <div>
                                        <p class="text-sm font-bold text-brand-text dark:text-white mb-1">Edukasi &
                                            Kepulangan</p>
                                        <ul class="space-y-1">
                                            <li class="text-xs text-gray-500 dark:text-white/50 flex items-center gap-2"><i
                                                    class="iconoir-clock text-brand-accent dark:text-brand-gold"></i> 07:00
                                                - Sarapan Dimsum</li>
                                            <li class="text-xs text-gray-500 dark:text-white/50 flex items-center gap-2"><i
                                                    class="iconoir-clock text-brand-accent dark:text-brand-gold"></i> 09:00
                                                - Kelas Merakit Lampion</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div
                                class="bg-emerald-50/50 dark:bg-emerald-500/5 p-4 rounded-xl border border-emerald-100 dark:border-emerald-500/10">
                                <p
                                    class="text-[10px] font-extrabold uppercase text-emerald-600 dark:text-emerald-400 mb-2">
                                    Termasuk</p>
                            ...
                            <div
                                class="bg-rose-50/50 dark:bg-rose-500/5 p-4 rounded-xl border border-rose-100 dark:border-rose-500/10">
                                <p class="text-[10px] font-extrabold uppercase text-rose-600 dark:text-rose-400 mb-2">Tidak
                                    Termasuk</p>
                    ...
                    <div
                        ...
                        <h4
                            class="text-xs font-extrabold uppercase tracking-widest text-brand-muted mb-6 border-b border-gray-100 dark:border-white/10 pb-2">
                            Logistik & Harga</h4>

                        <div class="space-y-4 mb-8">
                            <div>
                                <p class="text-[10px] uppercase text-brand-muted mb-0.5">Status Penjualan</p>
                                <p class="text-sm font-bold text-emerald-500 flex items-center gap-1"><i
                                        class="iconoir-check-circle"></i> Aktif (Tersedia)</p>
                            </div>
                            <div>
                        ...
                        <div
                            class="mt-auto bg-gray-50/80 dark:bg-white/5 rounded-2xl p-5 border border-gray-100 dark:border-white/10">
                            <p class="text-xs text-brand-muted line-through mb-0.5">Rp 1.200.000</p>
                            <p
                                class="font-serif text-3xl font-extrabold text-brand-text dark:text-brand-gold leading-none">
                                Rp 850.000</p>
    ...
    <div id="archiveModal"
        ...
        <div class="absolute inset-0 bg-brand-dark/80 backdrop-blur-sm cursor-pointer"
            onclick="closeModal('archiveModal')"></div>
        <div id="archiveModalCard"
            ...
            <div
                class="w-20 h-20 rounded-full bg-amber-50 dark:bg-amber-500/10 flex items-center justify-center text-amber-500 mx-auto mb-5">
                <i class="iconoir-archive text-4xl"></i>
            </div>

            <h3 class="font-bold text-brand-text dark:text-white text-xl mb-2">Nonaktifkan Paket?</h3>
            <p class="text-sm text-brand-muted dark:text-white/60 mb-8 leading-relaxed">Anda yakin ingin menonaktifkan
                paket wisata ini? Paket akan disembunyikan dari halaman pengunjung namun riwayat pemesanan masa lalu tetap
                aman.</p>

            <div class="flex flex-col gap-3">
                <button type="button"
                    class="w-full py-3.5 rounded-xl bg-amber-500 text-white font-bold text-sm shadow-lg shadow-amber-500/30 hover:bg-amber-600 transition-colors outline-none border-none cursor-pointer">
