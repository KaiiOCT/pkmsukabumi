...
@section('title', $package->title_line1 . ' ' . $package->title_line2 . ' | Paket Wisata Kampoeng Naga')
@section('meta_description', $package->catchphrase)
@section('content')
    ...
    <section
        ...
        <div class="relative max-w-7xl mx-auto px-5 sm:px-8 z-10 reveal">
            <div class="flex items-center gap-2 text-sm text-white/60 flex-wrap font-medium mb-8">
                ...
                <i class="iconoir-nav-arrow-right text-sm opacity-50"></i>
                <span class="text-brand-gold truncate max-w-[200px]">{{ $package->title_line1 }}
                    {{ $package->title_line2 }}</span>
            </div>
            ...
            <div class="max-w-4xl">
                <div class="flex flex-wrap gap-2 mb-5">
                    ...
                    <span class="badge badge-white">
                        <i class="iconoir-moon-sat text-xs"></i> {{ $package->duration }}
                    </span>
                    <span class="badge bg-white/10 text-white border border-white/20 backdrop-blur-md">
                        <i class="iconoir-map-pin text-xs text-brand-gold"></i> {{ $package->location_label }}
                    </span>
                </div>
                <h1 class="font-serif text-4xl md:text-5xl lg:text-[56px] font-bold text-white leading-tight mt-0 mb-4"
                    ...
                    {{ $package->title_line1 }} <br><span class="text-brand-gold italic">{{ $package->title_line2 }}</span>
                </h1>
    ...
    <section class="px-5 sm:px-8 relative z-20 -mt-24 md:-mt-32 mb-12">
        <div class="max-w-7xl mx-auto">
            <div
                class="grid grid-cols-2 md:grid-cols-4 gap-3 h-[300px] md:h-[400px] lg:h-[500px] rounded-3xl overflow-hidden shadow-card-hover border-4 border-white dark:border-[#1E1212]">
                <a href="{{ asset('storage/' . $package->main_image) }}" data-fancybox="paket-galeri"
                    data-caption="Vihara Widhi Sakti — Jantung Odeon"
                    class="col-span-2 row-span-2 relative overflow-hidden group cursor-zoom-in bg-brand-dark block">
                    <img src="{{ asset('storage/' . $package->main_image) }}" alt="Vihara Widhi Sakti — Jantung Odeon"
                    ...
                    <div
                        class="absolute top-4 right-4 w-10 h-10 bg-black/30 backdrop-blur-md rounded-full flex items-center justify-center text-white opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                        <i class="iconoir-zoom-in text-lg"></i>
                    </div>
                </a>
                @foreach ($package->gallery as $photo)
                    <a href="{{ asset('storage/' . $photo) }}" data-fancybox="paket-galeri"
                        class="relative overflow-hidden group cursor-zoom-in bg-brand-dark block">
                        ...
                        <div
                            class="absolute top-3 right-3 w-8 h-8 bg-black/30 backdrop-blur-md rounded-full flex items-center justify-center text-white opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                            <i class="iconoir-zoom-in text-sm"></i>
                        </div>

                        @if ($loop->last)
                            <div
                                class="absolute inset-0 bg-brand-dark/50 flex items-center justify-center backdrop-blur-sm group-hover:bg-brand-dark/40 transition-all">
        ...
        <div class="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16">
            ...
            <div class="lg:col-span-7 xl:col-span-8 space-y-14">
                ...
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 py-2 reveal">
                    <div
                        class="flex items-center gap-3.5 bg-brand-accent/5 p-4 rounded-2xl border border-brand-accent/10 hover:bg-brand-accent/10 transition-colors duration-300">
                        ...
                        <div class="flex flex-col justify-center">
                            <span
                                class="text-[10px] font-bold uppercase tracking-wider text-brand-muted dark:text-white/50 mb-0.5">Durasi</span>
                            <span
                                class="text-sm font-bold text-brand-text dark:text-white leading-none">{{ $package->duration }}</span>
                        </div>
                    </div>
                    <div
                        class="flex items-center gap-3.5 bg-brand-accent/5 p-4 rounded-2xl border border-brand-accent/10 hover:bg-brand-accent/10 transition-colors duration-300">
                        ...
                        <div class="flex flex-col justify-center">
                            <span
                                class="text-[10px] font-bold uppercase tracking-wider text-brand-muted dark:text-white/50 mb-0.5">Ukuran
                                Grup</span>
                            <span
                                class="text-sm font-bold text-brand-text dark:text-white leading-none">{{ $package->group_size }}</span>
                        </div>
                    </div>
                    <div
                        class="flex items-center gap-3.5 bg-brand-accent/5 p-4 rounded-2xl border border-brand-accent/10 hover:bg-brand-accent/10 transition-colors duration-300">
                        ...
                        <div class="flex flex-col justify-center">
                            <span
                                class="text-[10px] font-bold uppercase tracking-wider text-brand-muted dark:text-white/50 mb-0.5">Pemandu
                                & Bahasa</span>
                            <span
                                class="text-sm font-bold text-brand-text dark:text-white leading-none">{{ $package->guide }}</span>
                        </div>
                    </div>
                    <div
                        class="flex items-center gap-3.5 bg-brand-accent/5 p-4 rounded-2xl border border-brand-accent/10 hover:bg-brand-accent/10 transition-colors duration-300">
                        ...
                        <div class="flex flex-col justify-center">
                            <span
                                class="text-[10px] font-bold uppercase tracking-wider text-brand-muted dark:text-white/50 mb-0.5">Aktivitas</span>
                            <span
                                class="text-sm font-bold text-brand-text dark:text-white leading-none">{{ $package->activity_type }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="reveal">
                    ...
                    <div class="space-y-6">
                        ...
                        @foreach ($package->itinerary as $index => $day)
                            <div
                                class="rounded-3xl border border-brand-accent/10 bg-white dark:bg-[#1E1212] shadow-sm hover:shadow-card transition-shadow overflow-hidden">

                                <div
                                    class="flex items-center gap-4 p-6 pb-5 border-b border-brand-accent/8 bg-gradient-to-r from-brand-accent/5 to-transparent">

                                    <div
                                        class="w-16 h-16 rounded-2xl bg-brand-accent text-white flex flex-col items-center justify-center shadow-md shrink-0">

                                        <span class="text-xs font-medium uppercase tracking-widest opacity-80">
                                            Hari
                                        </span>
                                    ...
                                    <div>
                                        ...
                                        <h3
                                            class="font-serif text-xl md:text-2xl font-bold text-brand-text dark:text-white m-0">
                                        ...
                                        <p
                                            class="text-sm text-brand-muted dark:text-white/50 mt-1.5 m-0 flex items-center gap-1.5 font-medium">

                                            Pukul {{ $day['time_range'] }}
                                ...
                                <div class="p-6 md:p-8 space-y-8">
                                    ...
                                    @foreach ($day['activities'] as $item)
                                        <div class="flex gap-5 relative">
                                            ...
                                            @if (!$loop->last)
                                                <div
                                                    class="absolute left-[27px] top-8 bottom-[-2rem] w-[2px] bg-brand-accent/15">
                                                </div>
                                            @endif
                                            ...
                                            <div class="shrink-0 flex flex-col items-center relative z-10">
                                                ...
                                                <div
                                                    class="w-14 h-8 rounded-lg bg-{{ $item['color'] }}/10 text-{{ $item['color'] === 'brand-gold' ? 'amber-600' : 'brand-accent' }} font-bold text-sm flex items-center justify-center border border-{{ $item['color'] }}/20">
                                            ...
                                            <div class="flex-1 pb-2">
                                                ...
                                                <h4
                                                    class="font-bold text-brand-text dark:text-white mb-2 text-base md:text-lg">

                                                    {{ $item['title'] }}

                                                </h4>

                                                <p class="text-sm text-brand-muted dark:text-white/60 leading-relaxed m-0">
                ...
                <div class="reveal">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div
                            class="rounded-3xl border border-emerald-200/60 dark:border-emerald-900/30 bg-emerald-50/40 dark:bg-emerald-900/10 p-8 shadow-sm">
                            <div class="flex items-center gap-3 mb-6">
                                <div
                                    class="w-10 h-10 rounded-xl bg-emerald-500 text-white flex items-center justify-center shadow-md shadow-emerald-500/20">
                                    <i class="iconoir-check-circle text-xl"></i>
                                </div>
                            ...
                            <ul class="space-y-4">
                                @foreach (explode(',', $package->included) as $item)
                                    <li
                                        class="flex items-start gap-3 text-[15px] font-medium text-emerald-800/80 dark:text-white/70">
                                        <i
                                            class="iconoir-check text-emerald-500 font-bold mt-0.5 shrink-0 text-lg"></i>{{ $item }}
                                    </li>
                                @endforeach
                            </ul>
                        </div>
                        ...
                        <div
                            class="rounded-3xl border border-rose-200/60 dark:border-rose-900/30 bg-rose-50/40 dark:bg-rose-900/10 p-8 shadow-sm">
                            <div class="flex items-center gap-3 mb-6">
                                <div
                                    class="w-10 h-10 rounded-xl bg-rose-500 text-white flex items-center justify-center shadow-md shadow-rose-500/20">
                                    <i class="iconoir-xmark-circle text-xl"></i>
                                </div>
                                <h3 class="font-bold text-rose-900 dark:text-rose-400 text-lg m-0">Tidak Termasuk</h3>
                            </div>
                            <ul class="space-y-4">
                                @foreach (explode(',', $package->excluded) as $item)
                                    <li
                                        class="flex items-start gap-3 text-[15px] font-medium text-rose-800/80 dark:text-white/70">
                                        <i
                                            class="iconoir-xmark text-rose-500 font-bold mt-0.5 shrink-0 text-lg"></i>{{ $item }}
                                    </li>
                                @endforeach
                            </ul>
                        </div>
            ...
            <div class="lg:col-span-5 xl:col-span-4">
                <div class="sticky top-28 reveal">
                    <div
                        class="bg-white dark:bg-[#1E1212] rounded-[2rem] shadow-[0_8px_30px_rgb(139,26,26,0.12)] border border-brand-accent/15 dark:border-white/5 overflow-hidden">
                        <div
                            class="bg-rose-50 dark:bg-rose-900/30 px-6 py-3 border-b border-rose-100 dark:border-rose-800 flex items-center justify-between">
                            <div
                                class="flex items-center gap-2 text-rose-600 dark:text-rose-300 text-xs font-bold uppercase tracking-wider">
                                Ketersediaan Terbatas</div>
                            <span
                                class="text-xs font-bold text-rose-800 dark:text-rose-200 bg-rose-200/50 dark:bg-rose-800/40 px-2 py-1 rounded-md">Sisa
                                {{ $package->slots_left }} Slot</span>
                        </div>

                        <div class="p-8 pb-6">
                            <p class="text-sm font-semibold text-brand-muted dark:text-white/40 line-through mb-1">Rp
                                {{ number_format($package->price_strike, 0, ',', '.') }}</p>
                            <div
                                class="font-serif text-[42px] font-extrabold text-brand-text dark:text-white leading-none flex items-baseline gap-1.5 mb-2">
                                Rp {{ number_format($package->price, 0, ',', '.') }}
                                <span class="text-base font-medium text-brand-muted dark:text-white/50 font-sans">/
                                    pax</span>
                            </div>
                        </div>
                            ...
                            <ul class="space-y-4">
                                ...
                                @foreach ($policies as $p)
                                    <li class="flex items-center gap-3.5">
                                        ...
                                        <span
                                            class="text-sm text-brand-text dark:text-white/80 font-medium leading-snug">{{ $p['text'] }}</span>
                                    </li>
                                @endforeach
                            </ul>
                        </div>
    ...
    <section class="py-24 px-5 sm:px-8 relative overflow-hidden mt-6 mb-0 reveal">
        ...
        <div class="relative max-w-4xl mx-auto text-center reveal">
            ...
            <p class="text-lg md:text-xl text-[#FAF7F2]/80 mb-10 max-w-2xl mx-auto leading-relaxed font-medium">
                Selain paket yang tersedia, kami juga melayani kustomisasi jadwal khusus rombongan besar, kunjungan
                korporasi, atau tur edukasi sekolah.
            </p>
    ...
    <div id="bookingModal"
        ...
        <div class="absolute inset-0 bg-brand-dark/80 backdrop-blur-sm cursor-pointer" onclick="closeBookingModal()">
        </div>

        <div
            class="relative w-full max-w-2xl bg-white dark:bg-[#1E1212] rounded-[2rem] shadow-2xl overflow-hidden flex flex-col max-h-[95vh] sm:max-h-[90vh]">
            ...
            <div
                ...
                <div>
                    ...
                    <p class="text-xs text-brand-muted dark:text-brand-gold/70 mt-0.5">{{ $package->title_line1 }}
                        {{ $package->title_line2 }}
            ...
            <div class="p-6 sm:p-8 overflow-y-auto custom-scrollbar flex-1">
                ...
                <form id="bookingForm" action="{{ route('booking.store') }}" method="POST">
                    ...
                    <input type="hidden" name="package_name"
                        value="{{ $package->title_line1 }} {{ $package->title_line2 }}">
                    <input type="hidden" name="package_price" value="{{ $package->price }}">

                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">
                        <div>
                            <label class="block text-xs font-bold text-brand-text dark:text-white mb-1.5">Nama Lengkap
                                <span class="text-rose-500">*</span></label>
                            <input type="text" name="name" required placeholder="Contoh: Budi Santoso"
                                class="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-[#0A0505] text-brand-text dark:text-white text-sm outline-none focus:ring-2 focus:ring-brand-accent/20">
                        </div>

                        <div>
                            <label class="block text-xs font-bold text-brand-text dark:text-white mb-1.5">Nomor WhatsApp
                                <span class="text-rose-500">*</span></label>
                            <input type="number" name="phone" required placeholder="Contoh: 081234567890"
                                class="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-[#0A0505] text-brand-text dark:text-white text-sm outline-none focus:ring-2 focus:ring-brand-accent/20">
                        </div>
                    </div>

                    <div class="mb-5">
                        <label class="block text-xs font-bold text-brand-text dark:text-white mb-1.5">Email
                            (Opsional)</label>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">
                        ...
                        <div>
                            <label class="block text-xs font-bold text-brand-text dark:text-white mb-1.5">Jumlah Peserta
                                <span class="text-rose-500">*</span></label>
                            <div class="relative">
                                <i
                                    class="iconoir-group absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 dark:text-white/40 pointer-events-none"></i>
                                <input type="number" name="pax" required min="2" value="2"
                                    class="w-full pl-11 pr-4 py-3 rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-[#0A0505] text-brand-text dark:text-white text-sm outline-none focus:ring-2 focus:ring-brand-accent/20">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="message" rows="3"
                            class="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-[#0A0505] text-brand-text dark:text-white text-sm outline-none focus:ring-2 focus:ring-brand-accent/20"></textarea>
                    </div>

                    <div class="flex flex-col-reverse sm:flex-row gap-3">
                        <button type="button" onclick="closeBookingModal()"
                            class="w-full sm:w-1/3 py-3.5 rounded-xl border border-gray-200 dark:border-white/10 bg-transparent text-gray-600 dark:text-white/60 font-bold text-sm hover:bg-gray-50 transition-colors">
                            Batal
                        </button>

                        <!-- 🔥 SUBMIT FIX -->
                        <button type="submit"
                            class="w-full sm:w-2/3 py-3.5 rounded-xl bg-brand-accent dark:bg-[#A91D1D] text-white font-bold text-sm shadow-lg hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2">
    ...
    <script>
        ...
        function sharePackage() {
            if (navigator.share) {
                navigator.share({
                    title: '{{ $package->title_line1 }} {{ $package->title_line2 }} | Paket Wisata',
